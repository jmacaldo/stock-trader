    ...
    return (
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        ...
        <div className="text-center py-10">
          ...
          <p className="text-gray-700 text-xs mt-1">Search for a stock and place your first trade</p>
        </div>
      </div>
    )
  }
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-gray-700">{symbols.length} position{symbols.length !== 1 ? 's' : ''}</span>
        </div>
        <div className="flex items-center gap-3">
          {loading && <span className="text-xs text-gray-600">Updating...</span>}
          {lastUpdated && !loading && (
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm min-w-[560px]">
          <thead>
            <tr className="text-xs text-gray-600 uppercase tracking-wide border-b border-gray-800/50">
          ...
          <tbody>
            {rows.map(({ sym, shares, avgCost, name, current, value, pnl, pnlPct }) => (
              <tr key={sym} className="border-t border-gray-800/40 hover:bg-gray-800/20 transition-colors">
                <td className="px-4 py-3">
                  <div className="font-mono font-bold text-white text-sm">{sym}</div>
                  <div className="text-xs text-gray-600 truncate max-w-[130px]">{name}</div>
                </td>
                <td className="text-right px-4 py-3 text-gray-300 tabular-nums">{shares < 1 ? shares.toFixed(6) : shares.toFixed(4)}</td>
                <td className="text-right px-4 py-3 text-gray-500 tabular-nums text-xs">{usd(avgCost)}</td>
                <td className="text-right px-4 py-3 text-gray-200 tabular-nums font-medium">{usd(current)}</td>
                <td className="text-right px-4 py-3 text-white tabular-nums font-semibold">{usd(value)}</td>
                <td className="text-right px-4 py-3">
                  <div className={`font-semibold tabular-nums ${pnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    {pnl >= 0 ? '+' : ''}{usd(pnl)}
                  </div>
          ...
          {symbols.length > 1 && (
            <tfoot>
              <tr className="border-t border-gray-700 bg-gray-800/30">
                ...
                <td className="text-right px-4 py-2.5 text-white font-bold tabular-nums">{usd(totalValue)}</td>
                <td className={`text-right px-4 py-2.5 font-bold tabular-nums ${totalPnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
