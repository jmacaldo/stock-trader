...
export default function Search({ onSelect }) {
  ...
  return (
    <div className="space-y-3">
        ...
        <form onSubmit={handleSubmit}>
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-gray-900 border border-gray-700 rounded-xl pl-10 pr-10 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 transition-colors text-sm"
            />
            {loading ? (
            ...
            {results.map((r) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <span className="font-mono font-bold text-white text-sm w-14 shrink-0">{r.symbol}</span>
                  <span className="text-gray-400 text-sm truncate">{r.name}</span>
                </div>
                <span className="text-xs text-gray-600 bg-gray-800 px-2 py-0.5 rounded shrink-0 ml-2">{r.type}</span>
              </button>
            ))}
          </div>
        )}
      ...
      <div>
        <p className="text-xs text-gray-600 uppercase tracking-wide mb-2 px-1">Popular</p>
        <div className="flex flex-wrap gap-1.5">
          {POPULAR.map((sym) => (
            <button
              key={sym}
              onClick={() => handleSelect(sym)}
              className="px-3 py-1 bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-gray-600 text-gray-300 hover:text-white rounded-full text-xs font-mono font-semibold transition-colors"
            >
              {sym}
            </button>
          ))}
